...
**Repositório**: github.com/raelmz/cinema-arcano (terceiro commit feito e pushado — `b9ec5f5`: migration inicial + downgrade Prisma + docs atualizados). **Módulo de Auth quase completo, ainda sem commit**: register, login, `/auth/me` protegida, `JwtAuthGuard` e `RolesGuard` implementadas e testadas ponta a ponta. Falta só `prisma/seed.ts` (ADMIN/GATE) antes do commit único combinado. Ver seção 4/5.
...
Justificativa completa de cada uma está em `docs/PROJETO.md`, seção 4 — **as decisões de registro por papel, granularidade de commit do módulo Auth, PrismaModule global, e a escolha de guard própria sem Passport ainda não foram transcritas para lá, fazer isso ao fechar o módulo Auth**.
...
│   │   ├── seed.ts           → AINDA NÃO CRIADO — próximo passo
│   │   └── migrations/
...
- [ ] **`RolesGuard` ainda não testada com rota/usuário real** — nenhuma rota usa `@Roles()` ainda; plano é testar organicamente quando existirem usuários ADMIN/GATE reais via seed, em vez de criar rota descartável só para teste
- [ ] `prisma/seed.ts` (cria 1 ADMIN, 2 CUSTOMERs, 1 GATE) — **próximo passo imediato, ainda não iniciado**
- [ ] **Módulo Auth não commitado ainda** — falta só o seed para fechar o módulo por completo
- [ ] `docs/PROJETO.md` — decisões recentes (registro por papel, granularidade de commit, PrismaModule global, guard sem Passport) ainda não transcritas
...
1. **Criar `backend/prisma/seed.ts`** — 1 ADMIN, 2 CUSTOMERs, 1 GATE, com senhas hasheadas via Argon2id (mesma lógica do `AuthService.register`, mas direto no banco via Prisma Client, sem passar pela API)
2. Configurar o script de seed no `package.json` (campo `prisma.seed` ou script `npm run seed`, a definir na próxima sessão) e rodar
3. Conferir no Prisma Studio se os 4 usuários foram criados corretamente, com os papéis certos
4. **Testar `RolesGuard` com um usuário ADMIN real** — pode ser reaproveitando a própria `/auth/me` temporariamente com `@Roles('ADMIN')` só pra validar, ou esperando a primeira rota real que precise disso (ex: publicar sessão de filme, que é do organizador/ADMIN) — decidir isso na próxima sessão
5. Commit único do módulo Auth completo: `feat(auth): implementa registro, login, guards JWT/RBAC e seed`
6. Atualizar `docs/PROJETO.md` com todas as decisões pendentes listadas na seção 2
7. Depois disso: seguir o plano dia-a-dia já fechado — Dia 2 é frontend de login/cadastro + início do módulo Catálogo (TMDb)
