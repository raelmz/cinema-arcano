...
**Repositório**: github.com/raelmz/cinema-arcano (Módulo de Auth — backend — commitado e concluído). **Nesta sessão o developer usou o Gemini Pro (não o Claude) para iniciar o Frontend**: roteamento, tema visual, formulários de login/cadastro e integração com o backend NestJS já estão implementados e funcionando. O commit dessas mudanças de frontend **ainda não foi feito**. O developer volta a usar o Claude a partir de agora. Ver seção 4/5 para os próximos passos imediatos.
...
| Frontend — Escopo "Esqueci a Senha" | Descartado | Foco no MVP estrito do desafio; exigiria SMTP e rotas extras fora do core avaliado (reserva/QR Code) |
...
> **Nota importante**: a árvore acima do `frontend/` foi descrita pelo Gemini na sessão anterior, com base no código gerado — mas **ainda não foi conferida/lida diretamente por esta sessão**. Antes de mexer nesses arquivos, vale dar uma olhada rápida neles pra confirmar que batem com o que está documentado aqui.
...
- [ ] **Conferir/ler o código gerado pelo Gemini no frontend antes de continuar** — esta sessão (Claude) ainda não viu os arquivos reais, só a descrição textual repassada pelo developer
- [ ] **Frontend Auth ainda não commitado** — próximo passo imediato depois da conferência
- [ ] Transcrever para `docs/PROJETO.md` as novas decisões de frontend desta sessão (roteamento, formulários, tema visual, escopo "esqueci a senha")
...
1. **Ler/conferir os arquivos reais do frontend** criados na sessão com o Gemini (`app/globals.css`, `app/services/api.ts`, `app/login/page.tsx`, `app/register/page.tsx`) — confirmar que o código bate com o que foi descrito e que roda sem erros antes de seguir.
2. **Definir e implementar a estratégia de pós-login** (redirecionamento após login/cadastro + como as demais páginas saberão que o usuário está autenticado).
3. **Commitar o frontend de Auth** — `git add . && git commit -m "feat(auth): implementa telas de login e cadastro no frontend" && git push` (ajustar mensagem conforme o que for revisado no passo 1).
4. Transcrever para `docs/PROJETO.md` as decisões novas de frontend (roteamento, formulários, tema visual, escopo "esqueci a senha").
5. Iniciar o **Módulo Catálogo**: página Home no Next.js listando filmes via TMDb + módulo correspondente no NestJS.
6. Lembrar que `POST /sessions` (publicar sessão de filme) é candidato natural para `@Roles('ADMIN')` — primeira aplicação "real" da `RolesGuard` fora de teste.
