...
**Repositório**: github.com/raelmz/cinema-arcano. Módulo Auth (backend + frontend, incluindo pós-login via Context API) **100% concluído e commitado**. **Módulo Catálogo iniciado nesta sessão**: backend (`MoviesModule`, integração TMDb) implementado, testado ponta a ponta via `Invoke-RestMethod` e **commitado**. Próximo passo real: **frontend do Catálogo** (Home listando filmes, busca, página de detalhes). Ver seção 4/5.
...
- [ ] Decisões do módulo Catálogo (backend) ainda não transcritas para `docs/PROJETO.md` — pendente, mesmo padrão das seções 4.14+
- [ ] Módulo Catálogo (frontend) não iniciado: Home listando filmes, busca, página de detalhes (`app/movies/[id]/page.tsx`)
...
1. **Módulo Catálogo (frontend)** — próximo passo imediato: `app/page.tsx` (Home) consumindo `GET /movies` para listar filmes populares; campo de busca consumindo `GET /movies/search?query=`; `app/movies/[id]/page.tsx` para detalhes via `GET /movies/:id`. Novas funções (`getMovies`, `searchMovies`, `getMovieDetails`) em `app/services/api.ts`, mesmo padrão das já existentes.
2. Lembrar que `POST /sessions` (publicar sessão de filme) é candidato natural para `@Roles('ADMIN')` — primeira aplicação "real" da `RolesGuard` fora de teste.
3. Depois do Catálogo: Módulo de Salas/Assentos/Sessões e início de Reserva (dia 3 do plano).
