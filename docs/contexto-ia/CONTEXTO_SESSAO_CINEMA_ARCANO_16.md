...
**Repositório**: github.com/raelmz/cinema-arcano. Módulo Auth (backend + frontend, incluindo pós-login via Context API) **100% concluído e commitado**. **Módulo Catálogo 100% concluído** (backend + frontend): Home listando populares, busca, página de detalhes — implementado, testado manualmente no navegador ponta a ponta (incluindo bug de CORS e bug visual de `Image` corrigidos nesta sessão), **ainda não commitado**. Próximo passo real: developer commitar o Catálogo (frontend) e depois seguir para Módulo de Salas/Assentos/Sessões. Ver seção 4/5.
...
| Convenção de arquivos entregues pela IA | Todo arquivo novo ou editado pelo Claude leva um comentário na primeira linha com o caminho real dentro do repo (ex: `// frontend/app/page.tsx`) | Facilita o developer localizar onde colar/substituir o arquivo no projeto, sem precisar perguntar toda vez |
...
- [ ] **Módulo Catálogo (frontend) ainda não commitado** — próxima ação do developer antes de seguir
- [ ] Decisões do módulo Catálogo (backend E frontend) ainda não transcritas para `docs/PROJETO.md` — pendente, mesmo padrão das seções 4.14+
...
1. **Commitar o Módulo Catálogo (frontend)** — trabalho desta sessão (Home, busca, detalhes, fix de CORS no `main.ts`, fix de `next.config.ts`) ainda está só no ambiente local do developer, não commitado.
2. Transcrever para `docs/PROJETO.md` as decisões do módulo Catálogo — backend (sessão anterior) e frontend (esta sessão), mesmo padrão das seções 4.14+.
...
4. Depois do Catálogo: Módulo de Salas/Assentos/Sessões e início de Reserva (dia 3 do plano).
