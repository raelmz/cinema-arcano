...
**Repositório**: github.com/raelmz/cinema-arcano. Módulos Auth e Catálogo **100% concluídos e commitados** (backend + frontend). `docs/PROJETO.md` já reflete isso (decisões 4.26 a 4.30). **Módulo Salas/Sessões: decisões de design fechadas, schema conferido, implementação ainda não iniciada.** Sala única com 40 assentos decidida (4.31); comportamento do módulo (tela própria do organizador, bloqueio de conflito de horário, só sessões futuras visíveis) decidido (4.32). Próximo passo real: implementar backend (migration/seed de Room+Seat, depois `SessionsModule`). Ver seção 4/5.
