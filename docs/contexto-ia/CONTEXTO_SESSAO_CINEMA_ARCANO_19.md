...
**Repositório**: github.com/raelmz/cinema-arcano. Módulos Auth e Catálogo **100% concluídos e commitados** (backend + frontend), incluindo os commits `feat(auth)`, `feat(auth): pós-login`, `feat(movies)`, `feat(catalog)` e `fix(backend): CORS` — todos já refletidos no changelog abaixo, nenhum trabalho fora de sincronia. `docs/PROJETO.md` já reflete isso (decisões 4.26 a 4.30). **Módulo Salas/Sessões: seed concluída; `SessionsModule` escrito, fix de DI aplicado, backend sobe sem erro — falta rodar o roteiro de teste manual e commitar.** Seed de `Room`/`Seat` (`prisma/seed.ts`, sala única "Cinema Arcano — Sala 1", 5×8 = 40 assentos, idempotente com `skipDuplicates`) escrita, rodada e confirmada. `SessionsModule` implementado (5 arquivos: `create-session.dto.ts`, `sessions.service.ts`, `sessions.controller.ts`, `sessions.module.ts`, `app.module.ts` atualizado). O erro de DI (`JwtAuthGuard` não resolvia `JwtService` dentro do `SessionsModule`) foi **corrigido e confirmado**: `auth.module.ts` passou a exportar `JwtModule` também, além de `JwtAuthGuard`/`RolesGuard`. Backend sobe limpo, com as 4 rotas de `SessionsController` mapeadas (`POST /sessions`, `GET /sessions`, `GET /sessions/:id`, `PATCH /sessions/:id/cancel`). Duas decisões técnicas da implementação (`roomId` implícito, fallback de 120min sem `durationMinutes`) **confirmadas pelo developer** — ver seção 2. Próximo passo real: rodar o roteiro de teste manual via `Invoke-RestMethod` (criar/sobrepor/listar/cancelar/listar) e, passando, commitar o módulo. Ver seção 4/5.
...
│       └── sessions/          → SessionsModule/Controller/Service/DTO (backend sobe OK, falta roteiro de teste + commit)
...
- [ ] Rodar o roteiro de teste combinado (criar sessão como ADMIN, tentar sobrepor → 409, listar pública, cancelar, listar de novo pra confirmar que sumiu) via `Invoke-RestMethod`
- [ ] Commitar o `SessionsModule` (backend) como unidade funcional coerente, depois do roteiro passar
...
1. **Rodar o roteiro de teste do `SessionsModule` via `Invoke-RestMethod`**: criar sessão como ADMIN; tentar criar outra sessão sobreposta na mesma sala (deve dar 409); listar `GET /sessions` (pública); cancelar via `PATCH /sessions/:id/cancel`; listar de novo e confirmar que a sessão cancelada não aparece mais.
2. Commitar o `SessionsModule` (backend) como unidade funcional coerente, seguindo o padrão de commit único por módulo já usado em Auth/Catálogo.
3. Transcrever as decisões novas do módulo Salas/Sessões (implementação do `SessionsModule`, `roomId` implícito, fallback de duração) para `docs/PROJETO.md`.
