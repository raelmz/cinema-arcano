...
- [ ] Plano dia-a-dia para os dias restantes do prazo ainda não montado
...
1. Escolher provedor de hospedagem gerenciada para backend + PostgreSQL (Railway, Render, Fly.io, Neon, etc. — nenhum escolhido ainda; vale decidir antes de começar a codar pra já configurar variáveis de ambiente pensando no deploy)
2. Montar plano dia-a-dia para os dias restantes do prazo (faltam ~5 dias até 25/08 13:03 — está atrasando, priorizar isso ainda nesta sessão ou logo na próxima)
3. Começar o código de fato: primeiro módulo sugerido é autenticação (User + JWT + Argon2id + RBAC), por ser pré-requisito de tudo o resto (reservas, tickets, portaria) — dentro dele, já aplicar o payload do JWT do ticket recém-fechado quando chegar no módulo de tickets
4. Depois de auth: módulo de catálogo (integração TMDb) e módulo de sessões/salas/assentos, que alimentam o mapa de reserva
