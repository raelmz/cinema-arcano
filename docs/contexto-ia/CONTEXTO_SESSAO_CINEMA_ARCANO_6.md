...
| Hospedagem gerenciada (backend + PostgreSQL) | Render, free tier | Melhor limite de free tier entre as opções (Railway, Fly.io, Neon) pra manter o projeto no ar de forma duradoura — é projeto de portfólio, não descartável. Trade-off aceito conscientemente: cold start no free tier (primeira requisição após inatividade é mais lenta); documentado em `docs/PROJETO.md` seção 4.11 |
...
- [x] Hospedagem gerenciada escolhida: Render (backend + PostgreSQL), free tier, trade-off de cold start documentado em `docs/PROJETO.md`
