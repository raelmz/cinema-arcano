...
**Repositório**: github.com/raelmz/cinema-arcano (terceiro commit feito e pushado — `b9ec5f5`: migration inicial + downgrade Prisma + docs atualizados). **Módulo de Auth em andamento, ainda sem commit** — código de register/login escrito nesta sessão, mas ainda não testado nem commitado — ver seção 4/5.
...
│   ├── src/
│   │   ├── app.module.ts      → atualizado: importa ConfigModule (global), PrismaModule, AuthModule
│   │   ├── prisma/
...
│   │       ├── auth.service.ts   → criado nesta sessão: register (hash Argon2id, força role CUSTOMER) e login (verify + JWT) — AINDA NÃO TESTADO
│   │       ├── auth.controller.ts → criado nesta sessão: POST /auth/register, POST /auth/login — AINDA NÃO TESTADO
│   │       └── auth.module.ts    → criado nesta sessão: registra JwtModule.registerAsync com secret via ConfigService
│   ├── docker-compose.yml    → sobe Postgres 16 local em container (testado, funcionando)
│   ├── .env                  → local, fora do Git (DATABASE_URL local + JWT_SECRET + JWT_EXPIRES_IN adicionados nesta sessão)
...
- [x] Decisão de fluxo de trabalho: passo a passo, contexto atualizado a cada marco relevante
...
- [x] **`PrismaService` e `PrismaModule` criados** (`@Global()`), registrados no `AppModule`, app testada subindo limpo (`npm run start:dev` sem erro)
- [x] **`@nestjs/jwt` e `@nestjs/config` instalados**
- [x] **`JWT_SECRET` e `JWT_EXPIRES_IN` adicionados ao `.env`** (secret gerado com `crypto.randomBytes`)
- [x] **`ConfigModule.forRoot({ isGlobal: true })` registrado no `AppModule`**
- [x] **`AuthModule` criado**, registrando `JwtModule.registerAsync` com secret/expiração vindos do `ConfigService`
- [x] **`AuthService` escrito**: `register()` (verifica e-mail duplicado, hash Argon2id, força `role: CUSTOMER`) e `login()` (busca usuário, `argon2.verify`, gera JWT com payload `{ sub, role }`)
- [x] **`AuthController` escrito**: `POST /auth/register`, `POST /auth/login`
- [ ] **Pendência de verificação**: confirmar se o campo de senha no model `User` do `schema.prisma` se chama `password` (nome assumido no `AuthService` — pode precisar de ajuste se for outro nome)
- [ ] `npm run start:dev` ainda não rodado com o `AuthModule` novo — precisa confirmar que sobe sem erro
- [ ] `POST /auth/register` ainda não testado manualmente (Insomnia/Postman/curl)
- [ ] `POST /auth/login` ainda não testado manualmente
...
- [ ] **Módulo Auth não commitado ainda** — commitar só quando register + login estiverem testados e funcionando ponta a ponta
...
1. **Conferir o nome do campo de senha no `schema.prisma`** (`password` vs. outro nome) e ajustar `auth.service.ts` se necessário
2. Rodar `npm run start:dev` e confirmar que a aplicação sobe sem erro com `AuthModule` incluído
3. Testar `POST /auth/register` manualmente (corpo: `name`, `email`, `password`) — confirmar que retorna `accessToken` + dados do usuário com `role: CUSTOMER`
4. Testar `POST /auth/login` manualmente — confirmar que retorna `accessToken` válido para um usuário já registrado
5. Testar caso de erro: e-mail duplicado no registro (`ConflictException`) e senha errada no login (`UnauthorizedException`)
6. Decidir se `GET /auth/me` entra agora (exige guard já pronta) ou é adiado pra depois do passo das guards
7. **`JwtAuthGuard`** (valida token em rotas protegidas) e **`RolesGuard`** + `@Roles()` (restringe por papel) — necessários antes de qualquer módulo futuro que precise restringir por ADMIN/GATE
8. **`prisma/seed.ts`** — cria 1 ADMIN, 2 CUSTOMERs, 1 GATE (requisito não-funcional do desafio, seção 5 do PROJETO.md)
9. Commit único: `feat(auth): implementa registro e login com JWT`
10. Atualizar `docs/PROJETO.md` com as decisões de registro por papel, granularidade de commit e PrismaModule global
11. Depois disso: seguir o plano dia-a-dia já fechado — Dia 2 é frontend de login/cadastro + início do módulo Catálogo (TMDb)

---

## 6. Perfil do developer (para calibrar o nível de explicação)

Israel Menezes de Andrade — Full Stack Jr, autodidata, já usou React/Next.js, Node, Supabase, TypeScript em produção (projetos: Linkael, Painel de Cadastro de Produtos, Roteirizador PetroKar). Confortável com IA como ferramenta de apoio no dia a dia. Prefere entender o porquê das decisões antes de aceitar sugestões, mas nesta sessão pediu para receber o código pronto direto, sem explicação prévia passo a passo — calibrar por sinais futuros se isso é preferência pontual (pressa) ou mudança de estilo permanente. Nunca tinha trabalhado em monorepo nem usado Docker antes desta etapa — ambos aprendidos do zero. Ambiente: Windows 11 com WSL2, PowerShell, VS Code.
...
| 20/08/2026 | Hospedagem escolhida (Render). Início do módulo Auth: decisão de registro por papel (só CUSTOMER livre) fechada. `class-validator`/`class-transformer`/`argon2` instalados. DTOs de register e login criados. Decisão de commitar o módulo Auth inteiro de uma vez tomada. |
| 20/08/2026 | `PrismaService`/`PrismaModule` criados e testados (app sobe limpo). Decisão de `PrismaModule` global tomada. `@nestjs/jwt` e `@nestjs/config` instalados. `JWT_SECRET`/`JWT_EXPIRES_IN` adicionados ao `.env`. `AuthModule`, `AuthService` (register + login) e `AuthController` escritos — ainda não testados nem commitados. Pendência aberta: confirmar nome do campo de senha no schema do `User`. |
